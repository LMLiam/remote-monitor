...
json_escape() {
  ...
  value="${value//$'\f'/\\f}"
  value="${value//$'\b'/\\b}"
  control_chars=($'\001' $'\002' $'\003' $'\004' $'\005' $'\006' $'\007' $'\013' $'\016' $'\017' $'\020' $'\021' $'\022' $'\023' $'\024' $'\025' $'\026' $'\027' $'\030' $'\031' $'\032' $'\033' $'\034' $'\035' $'\036' $'\037')
  control_escapes=('\u0001' '\u0002' '\u0003' '\u0004' '\u0005' '\u0006' '\u0007' '\u000b' '\u000e' '\u000f' '\u0010' '\u0011' '\u0012' '\u0013' '\u0014' '\u0015' '\u0016' '\u0017' '\u0018' '\u0019' '\u001a' '\u001b' '\u001c' '\u001d' '\u001e' '\u001f')
  for i in "${!control_chars[@]}"; do
    value="${value//${control_chars[$i]}/${control_escapes[$i]}}"
  done
  printf '%s' "${value}"
}
