...
func buildGPURows(state core.AppState, valueWidth, activityWidth int, condensed bool) []TableRowSpec {
	...
	for idx, gpu := range s.GPUs {
		...
		rows = append(rows, TableFullRow(fmt.Sprintf("GPU%d PState", gpu.Index), ansi.Lav, "", "", chipCell(strings.ToUpper(fallbackString(gpu.PState, TextNA)), valueWidth, pStateBackground(gpu.PState)), pStateMeaning(gpu.PState), ansi.Lav, ""))
		...
		if !condensed {
			reasons := fallbackString(gpu.ThrottleReasons, TextNone)
			...
			rows = append(rows, TableFullRow(fmt.Sprintf("GPU%d Throttle", gpu.Index), SeverityColor(throttleSev), "", "", chipCell(strings.ToUpper(reasons), valueWidth, severityBackground(throttleSev)), throttleNote, SeverityColor(throttleSev), ""))
		}
	}
